...
class MetaLearner(object):
    def __init__(self, train_data, opt):
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=opt.meta_lr, weight_decay=opt.l2)
    ...
    def train_tasks(self):
        ...
        for batch in tqdm(sampler):
            ...
            for support_data, query_data in batch:
                ...
                for data in support_loader:
                    ...
                    loss, _, _ = forward(self.model, data)
                    loss.backward()
                    self.model.optimizer.step()

                # validate on query set and accumulate loss
            ...
            if task_num > 0:
                avg_loss = total_loss / task_num
                print(f"Meta batch avg query loss: {avg_loss.item():.4f}")
